...
class NumAddModelForm(BootStrapModelForm):
    ...
    def clean_mobile(self):
        ...
        exist = models.GoodNumber.objects.filter(mobile=txt_mobile).exists()
...
class NumEditModelForm(BootStrapModelForm):
    ...
    def clean_mobile(self):
        ...
        txt_mobile = self.cleaned_data["mobile"]
        exist = models.GoodNumber.objects.exclude(id=self.instance.pk).filter(mobile=txt_mobile).exists()
        if len(txt_mobile) != 11:
            # 验证不通过，抛出错误
            raise ValidationError("手机号格式错误")
        elif exist:
